...
drw_colored_text(Drw *drw, ClrScheme *scheme, int numcolors, int x, int y, unsigned int w, unsigned int h, char *text)
{
	...
	while (*ptr) {
		...
		c = *ptr;
		*ptr = 0;
		if (i)x = drw_statusbar_text(drw, x, y, w, h, buf, 0) ;
			//x = drw_statusbar_text(drw, x, y, w, h, buf, 0) + drw->fonts[0]->h;
		*ptr = c;
		drw_setscheme(drw, &scheme[c-1]);
		buf = ++ptr;
	}
	drw_statusbar_text(drw, x, y, w, h, buf, 0);
}

int //no half h padding before text and after
drw_statusbar_text(Drw *drw, int x, int y, unsigned int w, unsigned int h, const char *text, int invert)
{
	char buf[1024];
	int tx, ty, th;
	Extnts tex;
